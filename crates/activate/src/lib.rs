...
fn unpack_journal_listing(resp: broker::ListResponse) -> anyhow::Result<Vec<JournalSplit>> {
    ...
    for resp in resp.journals {
        ...
        v.push(JournalSplit {
            name: spec.name,
            append_bound: spec.append_bound,
            flags: spec.flags,
            labels: set,
            mod_revision: resp.mod_revision,
...
fn task_changes<'a>(
    ...
    } in shards
    {
        ...
        let (recovery_flags, recovery_replication) = if shard_spec.disable {
            // Pass through `flags` and `replication` to not awaken a suspended journal.
            (recovery_split.flags, recovery_split.replication)
        } else {
            // Clobber `flags` and `replication` to resume the journal if it's suspended.
            (template.recovery.flags, template.recovery.replication)
        };

        let recovery_spec = JournalSpec {
            name: recovery_name,
            append_bound: recovery_split.append_bound,
            flags: recovery_flags,
            replication: recovery_replication,
...
fn partition_changes(
    ...
    } in partitions
    {
        ...
        // We pass-through `flags` and `replication` of journals to preserve
        // their suspension state. Automated resumption happens within the
        // data-plane just prior to an append into the collection partition.
...
fn ops_journal_changes(spec: Option<JournalSpec>, mut splits: Vec<JournalSplit>) -> Option<Change> {
    let Some(mut spec) = spec else {
        return None;
    };

    // We expect zero or one splits.
    let split = splits.pop().unwrap_or_default();

    // If the journal exists and is not suspended, there's nothing to do (we don't update it).
    if (spec.replication, spec.flags) == (split.replication, split.flags) {
        return None;
    }

    // We'll create or resume the journal.
    spec.append_bound = split.append_bound;
...
fn map_partition_to_split(parent: &JournalSplit) -> anyhow::Result<(JournalSplit, JournalSplit)> {
    ...
    Ok((
        JournalSplit {
            ...
            append_bound: parent.append_bound,
            flags: parent.flags,
            labels: lhs_labels,
        ...
        // Case: creation and updates of ops collection partitions.
        {
            ...
            insta::assert_json_snapshot!(
                ...
                json!([
                    ...
                    "update-suspended",
                    ops_journal_changes(Some(spec.clone()), vec![suspended]),
                    "update-active",
                    ops_journal_changes(Some(spec.clone()), vec![active]),
                ])
            );
        }
    }
